...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="Notes optionnelles pour le numéro"
        )
        parser.add_argument(
            "--admin-user",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if action == "list":
            ...
            if not options["phone"]:
                raise CommandError("Le numéro de téléphone est requis pour cette action")
            ...
            if action == "add":
                self.add_phone(normalized_phone, options["notes"], options["admin_user"])
            elif action == "remove":
    ...
    def list_whitelist(self):
        ...
        whitelist = PhoneWhitelist.objects.select_related("added_by").order_by("-added_at")
        
        if not whitelist.exists():
            self.stdout.write(
                self.style.WARNING("Aucun numéro dans la liste blanche.")
            )
            return
        ...
        try:
            ...
            if created:
                ...
                if not whitelist_item.is_active:
                    ...
                    whitelist_item.save()
                    self.stdout.write(
                        self.style.SUCCESS(f"✅ Numéro {phone} réactivé dans la liste blanche.")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"⚠️  Numéro {phone} déjà présent dans la liste blanche.")
                    )
    ...
    def remove_phone(self, phone):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"⚠️  Numéro {phone} non trouvé dans la liste blanche.")
            )
        except Exception as e:
    ...
    def check_phone(self, phone):
        ...
        if is_authorized:
            ...
            admin_name = whitelist_item.added_by.get_full_name() or whitelist_item.added_by.phone
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Numéro {phone} NON AUTORISÉ")
            )
    ...
    def get_admin_user(self, admin_user_identifier):
        ...
        if not admin_user.is_staff:
            raise CommandError(f"L'utilisateur {admin_user.phone} n'est pas un administrateur.")
